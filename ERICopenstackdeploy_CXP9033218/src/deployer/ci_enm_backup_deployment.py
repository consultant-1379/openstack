"""This file contains logic for CI ENM backup."""
...
class CIENMBackupDeployment(Command):
    """
    ENM backup deployment.

    Attributes:
        cis_access (boolean): ENM cis access, defaults to True
    ...
    def __init__(self, *args, **kwargs):
        """Initialize a CIENMBackup Command object."""
    ...
    def get_parser(self, prog_name):
        ...
        Arguments:
            prog_name (str): Deployer ci enm backup deployment

        Returns:
            parser (object): list parameters for 'deployer ci enm backup deployment --help'
            usage: deployer ci enm backup deployment [-h] --deployment-name [DEPLOYMENT_NAME]
    ...
    def take_action(self, args):
        ...
        workflow.wait_for_workflow_definition(
            workflow_id=f'enmdeploymentworkflows.--.{workflows_version}.--.BackupDeployment__top'
        )
        workflow.execute_workflow_and_wait(
            workflow_name='Backup Deployment',
            workflow_id=f'enmdeploymentworkflows.--.{workflows_version}.--.BackupDeployment__top',
            workflow_data={"tag": {"type": "String", "value": f'{args.backup_tag}'}},
            max_check_attempts=args.workflow_max_check_attempts
        )
        utils.remove_temporary_directory()
        LOG.info(
            'The Backup Deployment workflow has successfully completed. Now follow the relevant \
